...
우리반 notion: [11기 서울 3반](https://www.notion.so/11-3-6af233f3dcce4e87b0b3da80316911e3?pvs=21) 

공용 문서: [](https://www.notion.so/5fef18bbcd4b467fb8c7ed819250e1e8?pvs=21) 

- 사전 학습
    - Step1 수학적 사고력 이해
        - 필수 학습
            - ******프로그래밍과 논리/수학(0)******
                1. 프로그래밍이 어려운 이유 1)문법과 라이브러리, 2)논리(Hard Logic)
                2. 수학적 귀납법: P(1)이 참이고, P(n) → P(n+1)이 참이면 P(n)은 모든 자연수 n에 대하여 참이다.
                **귀납법을 증명함에 있어서는 전건인 P(n)의 참/거짓 여부를 판단할 필요가 없다. 참이라고 가정한 뒤, n=1 또는 n=k일 때 명제가 참이 되는지만 보이면 됨**
                3. 강한 형태의 수학적 귀납법: P(1)이 참이고, P(1)^P(2)^P(3)^…^P(n) → P(n+1)이 참이면 P(n)은 모든 자연수 n에 대해서 참이다.
                4. 버블 소트의 증명
                배열 A(1), A(2), … ,A(3)를 소팅하는 알고리즘의 정확성을 증명하려고 한다면, 증명이 가능한 명제는 다음과 같을 것: “A(1) < A(2) < A(3) < … < A(n)”
            - ********************************************************논리와 증명/수와 표현(1,2)********************************************************
                1. 논리와 증명
                
                [SW Expert Academy](https://swexpertacademy.com/main/learn/course/lectureHtmlViewer.do#none)
                
                1. 수와 표현
                이진법에서 k개의 비트를 사용하면 0부터 2^k-1까지 표현 가능. (10진수로 k자리를 쓰면 0~10^k-1까지 표현할 수 있는 것과 동일)
                    
                    ****************************************************************************************컴퓨터 분야에서 로그의 밑은 항상 2****************************************************************************************
                    
                    log n이란 n을 표현하는 데 몇 비트가 필요한가의 답
                    
                    x = log n을 비교하면 x가 더 작고, n이 커질수록 엄청나게 달라진다. (**********************************ex. 32비트 컴퓨터의 주소 공간은 2의 32승 = 약 40억개 주소**********************************)
                    
        - 자율 학습
            - ************************************집합과 조합론/기초수식(3,4)************************************
            - ****************************************************재귀/동적 프로그래밍(5,6)****************************************************
            - **************************************************************************************************조합론/기초 알고리즘 프로그래밍 과제(7,8)**************************************************************************************************
    - Step2 트랙별 언어 특성 및 기본 문법 이해
        - ****************************************************왕초보를 위한 python: 쉽게 풀어 쓴 기초 문법과 실습****************************************************
            ...
            - 파이썬 맛보기
            덧셈 +, 뺄셈 -, 곱셈*, 나눗셈/ (근사치//), 나머지%, 몫과 나머지 한 번에 구하기 divmod(50, 8)
            - **변수 = ‘문자열’**
            a = ‘pig’ → a라는 변수에 ‘pig’라는 문자열을 넣으라는 뜻.
            **’ ’가 없는 것은 변수, ‘ ‘가 있는 것은 문자열이다.**
            
            변수를 정의한 다음 + 로 표현하면 두 문자열을 이어붙인 결과가 나온다.
            
            변수 사이에 ****************************************************공백 문자 ‘ ‘를 추가****************************************************하면 단어 사이에 빈칸이 생긴다.
            - ************리스트 [ , , ]************
            - ************리스트 원소 개수 len()************
            - ************리스트 원소 순서 family[숫자]************
            family = [’miracle’, ‘mason’, ‘esther’, ‘noni’]
            len(family)
            family(3)
            **************************************************************프로그래밍에서 첫번째 순서는 늘 0이므로, 3을 입력하면 ‘noni’가 출력된다.**************************************************************
            - **************************************************리스트에서 원소 빼기 family.remove(’원소이름’)**************************************************
            - 인터프리터와 컴파일러
            인터프리터interpret = 동시통역 → 파이선 쏄은 interpreter다.
            컴파일러complie = 순차통역
            - ㅇ
            1. 제어 구조
            - while
            ...
            1. 함수
            - 함수 만들기
            def print_list(a) → print_list()라는 함수를 만들겠다는 의미.
            - 반환문
            - 지역변수, 전역변수
            - 람다
            1. 데이터 타입
